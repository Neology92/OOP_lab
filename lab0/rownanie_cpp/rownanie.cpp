...
{
  cout << "Trojmian: " << TKw.a << "x^2 " << (TKw.b>=0? "+ " : "- ")
       << abs(TKw.b) << "x " << (TKw.c>=0? "+ " : "- ") << abs(TKw.c) << endl;
}
 ...
 *  0 - brak rozwiazan w zbiorze liczb rzeczywistych.
 */
...
int main() {
  ...
  if (TKw.a == 0) {
    ...
    cout << "\nErr: To nie jest rownanie kwadratowe!\n";
  ...
  switch (WyliczPierwiastki(TKw,&X1,&X2)) {
    ...
    case 0:
      cout << "Brak rozwiazan w zbiorze liczb rzeczywistch\n";
      break;
  }
